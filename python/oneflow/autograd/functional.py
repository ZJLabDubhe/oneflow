...
def _autograd_grad(outputs, inputs, grad_outputs=None, create_graph=False, retain_graph=None):
    ...
    if len(new_outputs) == 0:
        ...
        return oneflow.autograd.grad(new_outputs, inputs, new_grad_outputs,retain_graph=retain_graph,
...
def vjp(
    ...
    with oneflow.set_grad_enabled(enable_grad):
        grad_res = _autograd_grad(outputs, inputs, v, create_graph=create_graph)#autograd.grad
...
def vhp(func, inputs, v=None, create_graph=False, strict=False):
    ...
    Example:
        ...
        >>> inputs = (oneflow.rand(2), oneflow.rand(2))
        >>> v = (oneflow.zeros(2), oneflow.ones(2))
